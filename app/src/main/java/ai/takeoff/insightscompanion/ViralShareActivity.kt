...
class ViralShareActivity : Activity() {
    ...
    private fun ensureEndpoint() {
        ...
        prefs.edit().putString("endpoint", PayloadClient.viralEndpoint(prefs.getString("endpoint", "").orEmpty())).apply()
    }
    ...
    private fun buildUi(): View {
        ...
        root.addView(LinearLayout(this).apply {
            ...
            addView(label("TAKEOFF • SERVER VIRAL AUTOPSY", 11f, accent, true))
            addView(label("کالبدشکافی سریع ریلز", 27f, Color.WHITE, true).apply { setPadding(0, dp(5), 0, 0) })
            addView(label("v${BuildConfig.VERSION_NAME} • بدون کلید • گوشی فقط لینک را می‌فرستد", 12f, muted, false).apply { setPadding(0, dp(7), 0, 0) })
        }, lp(dp(14)))
        ...
        root.addView(card().apply {
            ...
            status = label("آماده…", 17f, Color.WHITE, true).apply { setPadding(0, dp(8), 0, dp(10)) }
            addView(status)
            percent = label("", 13f, accent, true).apply { visibility = View.GONE; setPadding(0, 0, 0, dp(5)) }
            addView(percent)
            mediaBar = ProgressBar(this@ViralShareActivity, null, android.R.attr.progressBarStyleHorizontal).apply {
                max = 100; progress = 0; isIndeterminate = false; visibility = View.GONE
            }
            addView(mediaBar, LinearLayout.LayoutParams(-1, dp(9)).apply { bottomMargin = dp(10) })
            stages = label("○ دریافت لینک\n○ دریافت رسانه روی سرور\n○ Whisper + تحلیل تصویر\n○ تحلیل رفتاری\n○ حافظه یادگیری", 13f, muted, false).apply {
                setLineSpacing(dp(3).toFloat(), 1.12f); background = rounded(panel2, 16); setPadding(dp(14), dp(12), dp(14), dp(12))
            }
            addView(stages)
            retry = Button(this@ViralShareActivity).apply {
                ...
                background = rounded(Color.rgb(29, 41, 54), 14, Color.rgb(53, 69, 86)); setOnClickListener { startAnalysis() }
            }
            addView(retry, LinearLayout.LayoutParams(-1, dp(50)).apply { topMargin = dp(9) })
        }, lp(dp(12)))

        root.addView(card().apply {
            addView(label("گزارش هوشمند", 12f, accent, true))
            addView(label("ویدیو، صدا، دیالوگ، Hook، داستان، تدوین، Retention و الگوها روی سرور تحلیل می‌شوند.", 12f, muted, false).apply { setPadding(0, dp(6), 0, dp(12)) })
            reportView = label("", 13.5f, Color.WHITE, false).apply { setLineSpacing(0f, 1.22f); setTextIsSelectable(true) }
            addView(reportView)
        })
        root.addView(Button(this).apply { text = "بستن"; isAllCaps = false; setTextColor(Color.WHITE); background = rounded(panel2, 14, border); setOnClickListener { finish() } }, LinearLayout.LayoutParams(-1, dp(50)).apply { topMargin = dp(14) })
        scroll.addView(root)
        return scroll
    }
    ...
    private fun startAnalysis() {
        ...
        retry.visibility = View.GONE
        reportView.text = ""
        status.text = "در حال ایجاد پردازش امن و بدون کلید…"
        status.setTextColor(Color.WHITE)
        stages.text = "✓ دریافت لینک\n● آماده‌سازی سرور\n○ Whisper + تحلیل تصویر\n○ تحلیل رفتاری\n○ حافظه یادگیری"
        val prefs = getSharedPreferences("takeoff_companion_plain", Context.MODE_PRIVATE)
        ...
        Thread {
            ...
            if (started == null) {
                runOnUiThread { showError("ارتباط اولیه با سرور برقرار نشد", "اینترنت را بررسی کن و دوباره تلاش کن.") }
                return@Thread
            }
            if (started.first !in 200..299) {
                val detail = if (started.first == 401) "این مسیر نباید کلید بخواهد. هیچ کدی وارد نکن؛ سرور باید به نسخه بدون کلید ارتقا پیدا کند." else "HTTP ${started.first} • ${safeServerErrorBody(started.second)}"
                runOnUiThread { showError("شروع پردازش سرور ناموفق بود", detail) }
                return@Thread
            }
            val root = runCatching { JSONObject(started.second) }.getOrNull()
            val jobId = root?.optString("job_id").orEmpty()
            val token = root?.optString("poll_token").orEmpty()
            if (jobId.isBlank() || token.isBlank()) {
                runOnUiThread { showError("پاسخ شروع پردازش معتبر نبود") }
                return@Thread
            }
    ...
    private fun poll(endpoint: String, jobId: String, token: String, myGeneration: Int) {
        ...
        while (generation == myGeneration && System.currentTimeMillis() < deadline) {
            ...
            if (pair == null) {
                failures++
                if (failures >= 8) {
                    runOnUiThread { if (generation == myGeneration) showError("ارتباط با وضعیت پردازش قطع شد", "پردازش هیچ کلیدی نمی‌خواهد. اینترنت را بررسی کن و دوباره تلاش کن.") }
                    return
                }
                Thread.sleep(1000)
                continue
            ...
            if (pair.first !in 200..299) {
                runOnUiThread { if (generation == myGeneration) showError("وضعیت پردازش قابل دریافت نیست", "HTTP ${pair.first} • ${safeServerErrorBody(pair.second)}") }
                return
            }
            ...
            when (root.optString("status")) {
                "completed" -> {
                    ...
                    runOnUiThread { if (generation == myGeneration) showCompleted(result) }
                    return
                }
                "partial" -> {
                    runOnUiThread { if (generation == myGeneration) showError("سرور فایل اصلی ویدیو را پیدا نکرد", root.optString("error_code", "server_side_media_unavailable")) }
                    return
                }
                "failed" -> {
                    runOnUiThread { if (generation == myGeneration) showError("دریافت یا تحلیل سرور ناموفق بود", root.optString("error_code", "server_processing_failed")) }
                    return
                }
            }
            Thread.sleep(800)
    ...
    private fun renderProgress(root: JSONObject) {
        ...
        if (stage in setOf("media", "preprocess", "analysis", "behavior", "learning", "completed") || pct >= 0) {
            mediaBar.visibility = View.VISIBLE; percent.visibility = View.VISIBLE
            if (pct in 1..100) {
                mediaBar.isIndeterminate = false; mediaBar.progress = pct
                percent.text = if (total > 0) "دریافت واقعی رسانه: $pct٪ • ${bytes(downloaded)} از ${bytes(total)}" else "دریافت واقعی رسانه: $pct٪ • ${bytes(downloaded)}"
            } else {
                mediaBar.isIndeterminate = true
                percent.text = if (downloaded > 0) "دریافت واقعی رسانه: ${bytes(downloaded)} • حجم کل از CDN اعلام نشده" else "در حال تعیین حجم واقعی رسانه…"
            }
        }
        when (stage) {
    ...
    private fun showCompleted(root: JSONObject) {
        ...
        percent.visibility = View.VISIBLE; percent.text = "دریافت واقعی رسانه: 100٪ ✓"
        stages.text = "✓ دریافت لینک\n✓ دریافت رسانه روی سرور — 100٪\n✓ Whisper + تحلیل تصویر\n✓ تحلیل رفتاری\n✓ حافظه یادگیری"
        status.setTextColor(accent)
        status.text = if (root.optBoolean("deduplicated", false)) "گزارش قبلی معتبر باز شد" else "تحلیل سروری کامل شد و وارد حافظه یادگیری شد"
    ...
    private fun resetProgress() {
        ...
        mediaBar.progress = 0; mediaBar.isIndeterminate = false; mediaBar.visibility = View.GONE
        percent.text = ""; percent.visibility = View.GONE
    }
